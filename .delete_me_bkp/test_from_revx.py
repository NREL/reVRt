...
def _run_cli(
    ...
    # TODO: Use fixture function here
...
@pytest.fixture
def ri_ba(revx_transmission_layers):
    """Generate test BA region"""
    with xr.open_dataset(
        revx_transmission_layers, consolidated=False, engine="zarr"
    ) as ds:
        iso_regions = ds["ISO_regions"]
        transform = ds.rio.transform()
        crs = ds.rio.crs

    s = rasterio.features.shapes(iso_regions, transform=transform)
    ba_str, shapes = zip(
        *[(f"p{int(v)}", shape(p)) for p, v in s if int(v) != 0], strict=True
    )
...
def test_regional_cli(
    ...
    ri_feats_path = tmp_path / "ri_feats.gpkg"
    ri_feats.to_file(ri_feats_path, driver="GPKG", index=False)

    ri_ba_path = tmp_path / "ri_ba.gpkg"
    ...
    out_fp = run_gaps_cli_with_expected_file("map-ss-to-rr", config, tmp_path)

    # -- Build Route Table --

    config = {
        "cost_fpath": str(revx_transmission_layers),
        "features_fpath": str(out_fp),
