...
def ba_regions_and_network_nodes():
    ...
    ba_str, shapes = zip(
        *[("p{}".format(int(v)), shape(p)) for p, v in s if int(v) != 0]
    )

    state = ["Rhode Island"] * len(ba_str)
    ri_ba = gpd.GeoDataFrame(
        {"ba_str": ba_str, "state": state},
        crs=profile["crs"],
...
def test_capacity_class(capacity, route_table):
    ...
    """
    truth = os.path.join(
        TESTDATADIR, "xmission", f"least_cost_paths_{capacity}MW.csv"
    )
    cap = _cap_class_to_cap(capacity)
    cost_layer = {"layer_name": f"tie_line_costs_{cap}MW"}
    test = LeastCostPaths.run(
        COST_H5,
        route_table,
        [cost_layer],
        max_workers=1,
...
def test_cost_multiplier_layer(route_table):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with h5py.File(cost_h5_path, "a") as fh:
            ...
            fh.create_dataset(
                "test_layer", data=np.ones(shape, dtype="float32") * 7
            )
...
def test_clip_buffer():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        out_no_buffer = LeastCostPaths.run(
            out_cost_fp,
            route_table_fp,
            [cost_layer],
            max_workers=1,
...
def test_cli(runner, save_paths, route_table):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        result = runner.invoke(main, ["from-config", "-c", config_path, "-v"])
...
def test_reinforcement_cli(runner, ba_regions_and_network_nodes, save_paths):
    ...
    conns = ri_feats[ri_feats["category"] == TRANS_LINE_CAT].reset_index(
        drop=True
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ri_network_nodes.to_file(
            ri_network_nodes_path, driver="GPKG", index=False
        )
        ...
        config = {
            ...
            "cost_layers": [
                {
                    "layer_name": "tie_line_costs_400MW",
        ...
        assert len(test) == 69
        assert np.isclose(
            test.reinforcement_cost_per_mw.min(), 7405.728, atol=0.001
        )
        assert np.isclose(test.reinforcement_dist_km.min(), 1.918, atol=0.001)
        assert np.isclose(
            test.reinforcement_dist_km.max(), 80.0236, atol=0.001
        )
        assert len(test["reinforcement_poi_lat"].unique()) == 4
        assert len(test["reinforcement_poi_lon"].unique()) == 4
        assert np.isclose(
            test.reinforcement_cost_per_mw.max(), 1213837.2737, atol=0.001
        )

    LOGGERS.clear()


def test_reinforcement_cli_single_tline_voltage(
    runner, ba_regions_and_network_nodes
):
    """
...
def test_apply_row_mult(runner, route_table):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        config = {
            ...
            "save_paths": False,
            "cost_layers": [
                {"layer_name": cost_layer, "apply_row_mult": True}
            ],
            "friction_layers": [DEFAULT_BARRIER],
        }
        config_path = os.path.join(td, "config.json")
...
def test_apply_mults_by_route(runner, route_table):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for __, row in test.iterrows():
            ...
            divisors.append(
                1.2 * row_config[voltage] * polarity_config[voltage][polarity]
            )
